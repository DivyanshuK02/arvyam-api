...
class TestFailClosed:
    ...
    def test_returns_original_on_invalid_triad_length(self):
        """Should return original unchanged if triad has wrong length."""
        from app.memory.rerank import rerank
        from app.memory.context import MemoryContext
        ...
        # Should return input unchanged (fail-closed)
        assert result.was_reranked is False
    
    def test_handles_empty_triad(self):
        """Should handle empty triad gracefully."""
        from app.memory.rerank import rerank
        from app.memory.context import MemoryContext
