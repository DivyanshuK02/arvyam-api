...
app = FastAPI(title="Arvyam API (MVP Simple Tiers)")
...
# Expect these files next to main.py
...
def classify_category(prompt: str) -> str:
    """Pick the best category from rules.json by keyword hits (simple).
    Fallback to Love/Encouragement/Gratitude, else first rule."""
    p = prompt.lower()
    best_cat = None
    best_hits = 0
    for r in RULES:
...
def shape_item(r: Dict[str, Any]) -> Dict[str, Any]:
    return {
        "id": r["id"],
        "title": r.get("title") or "Curated Bouquet",
...
def pick_three(category: str) -> List[Dict[str, Any]]:
    """Return exactly 3 items for the category, aiming for low/mid/high by price.
    Enforce distinct images when possible (best-effort)."""
    # Filter by category tag
    candidates = [c for c in CATALOG if category in (c.get("tags") or [])]
    if not candidates:
        candidates = CATALOG[:]  # safe fallback

    # Sort by price low→high
    candidates.sort(key=lambda x: x.get("price_inr", 999999))

    # Choose target prices (low/mid/high)
    ...
    # pick for each target
    for p in targets:
        item = first_at(p)
        if item and item not in chosen:
            chosen.append(item)

    # pad to 3 with next that has a new image
    for c in candidates:
...
def refine_copy(items: List[Dict[str, Any]], category: str) -> List[Dict[str, Any]]:
    """Make the 3 cards feel different by tier labels only (no flower names)."""
    ...
    for it in items:
        p = it["price"]
        base_title = it["title"].split(" • ")[0]  # avoid stacking suffixes
        if p == low:
