...
# Ultra-beginner-safe Selection Engine (Refreshed, Tier-Ordered)
...
# - Final ordering: Classic → Signature → Luxury (LG sits within Luxury); MONO "floats" by its tier
#
# LG policy:
#   The engine reads rules/tier_policy.json.
#   If allowed_emotions is empty, LG is allowed for all emotions except those in blocked_emotions.
...
def order_by_tier(items: List[Dict[str,Any]]) -> List[Dict[str,Any]]:
    """Classic → Signature → Luxury; within Luxury, put LG after non-LG."""
    def key(it: Dict[str,Any]):
        tier_rank = TIER_RANK.get(it.get("tier"), 99)
...
def enforce_lg_cap(triad: List[Dict[str,Any]], mix_pool: List[Dict[str,Any]], mono_pool: List[Dict[str,Any]]) -> List[Dict[str,Any]]:
    ...
    for i in order:
        ...
        if rep:
            # Replacement mapped without note by default; we'll reattach note below if needed
...
def selection_engine(prompt: str, context: Optional[Dict[str,Any]]=None) -> List[Dict[str,Any]]:
    ...
    # -- Map & assemble (preserve editorial note on MIX) --
    raw_mix = [_map_out(it, note=note) for it in mix_picks]
    mono_out = _map_out(mono_pick)

    # Unsorted triad
    triad = [raw_mix[0], raw_mix[1], mono_out]

    # Enforce LG cap (may replace an item from pools)
    triad = enforce_lg_cap(triad, mix_pool, mono_pool)

    # If we had an editorial note, ensure any replaced MIX keeps the note
    if note:
        for card in triad:
            if (not card.get("mono", False)) and ("note" not in card):
                card["note"] = note

    # Final tier sort: Classic → Signature → Luxury (LG sits within Luxury)
    triad = order_by_tier(triad)
